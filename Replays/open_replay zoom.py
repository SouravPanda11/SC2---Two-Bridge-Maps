...
INLINE_PRE_START_DELAY_SECONDS = 4.0
...
SC2_CAMERA_RENDER_SIZE = (1280, 720)
SC2_FEATURE_CAMERA_WIDTH = 24.0
SC2_RENDER_CAMERA_WIDTH = 96.0
SC2_OBSERVER_CAMERA_DISTANCE = 120.0

def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Open a saved PPO replay in the built-in SC2 replay window only. "
            "This zoom variant requests a single main render view, attempts to "
            "zoom the observer camera out to show more of the map, and does not "
            "open the separate PySC2 Starcraft Viewer window."
        )
    )
    parser.add_argument(
        "replay",
    ...
    parser.add_argument(
        "--render-camera-width",
        type=float,
        default=SC2_RENDER_CAMERA_WIDTH,
        help="Requested RGB camera width in world units. Larger values zoom out more.",
    )
    parser.add_argument(
        "--observer-camera-distance",
        type=float,
        default=SC2_OBSERVER_CAMERA_DISTANCE,
        help="Requested observer camera distance. Larger values zoom out more if supported.",
    )
    parser.add_argument(
        "--lock-camera",
        action="store_true",
        help="Keep the observer camera pinned to the map center during playback.",
    )
    parser.add_argument(
        "--pysc2-python",
...
def build_pysc2_command(
    ...
    cmd.extend(
        [
            ...
            f"--render-camera-width={args.render_camera_width}",
            f"--observer-camera-distance={args.observer_camera_distance}",
        ]
    )
...
def replay_camera_center(controller, point):
    info = controller.game_info()
    if not info.HasField("start_raw"):
        raise RuntimeError("Replay game info is missing start_raw, cannot compute the replay camera center.")

    playable = info.start_raw.playable_area
    p0 = point.Point.build(playable.p0)
    p1 = point.Point.build(playable.p1)
    if p0 != p1:
        return (p0 + p1) / 2

    return point.Point.build(info.start_raw.map_size) / 2


def build_observer_camera_action(sc_pb, camera_center, distance: float):
    action = sc_pb.ObserverAction()
    camera_center.assign_to(action.camera_move.world_pos)
    if distance > 0:
        action.camera_move.distance = distance
    return action
...
def run_pysc2_replay(args: argparse.Namespace) -> int:
    ...
    interface.feature_layer.width = SC2_FEATURE_CAMERA_WIDTH
    ...
    interface.render.width = args.render_camera_width
    interface.render.crop_to_playable_area = True
    ...
    with run_config.start(
        ...
        controller.observe()
        camera_center = replay_camera_center(controller, point)
        observer_camera_action = build_observer_camera_action(
            sc_pb,
            camera_center,
            args.observer_camera_distance,
        )
        controller.observer_act(observer_camera_action)
...
def main() -> int:
    ...
    print("Mode: single windowed SC2 replay window only; zoom-out attempt with main screen only.")
