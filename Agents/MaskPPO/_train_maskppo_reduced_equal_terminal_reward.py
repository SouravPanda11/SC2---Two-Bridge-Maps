"""
Isolated Reduced MaskPPO driver for equal terminal-success rewards.

The established per-map trainer is reused without modifying it. Environment
factories passed to SubprocVecEnv capture the new equal-25 class explicitly,
...
AGENT_NAME = "MaskPPO_NS_reduced_equal_terminal_reward_25"
...
MAP_ENV_MODULES = {
    "V1_Base": (
        ...
        "V1_Base_equal_terminal_reward_NS"
    ),
    "V2_Navigate": (
        "Environments.NS_AM_RM_mean_reduced."
        "V2_Navigate_equal_terminal_reward_NS"
    ),
}
    ...
    def make_env(rank, base_seed, visualize=False, realtime=False):
        def _init():
            ...
            worker_tmp_dir = tempfile.mkdtemp(
                prefix=f"tbm-maskppo-equal-reward-worker-{rank}-"
            )
            atexit.register(
...
def train_with_settings(
    *,
    ...
    if map_name not in TRAIN_MODULES:
        ...
        raise ValueError(
            "Equal-reward Reduced MaskPPO supports only "
