...
from Environments.TB_env_SF_AM_RM_V2_Base import TwoBridgeEnv
...
class FlattenActionWrapper(Wrapper):
    ...
    def __init__(self, env):
        ...
        self.action_space = spaces.MultiDiscrete([3] + [2]*5 + [9] + [6])
         ...
         # -------- observation space: upgrade mask ----
        full_len = int(self.action_space.nvec.sum())      # 28

        obs_space = deepcopy(self.env.observation_space)  # <-- fix here
        obs_space.spaces["action_mask"] = spaces.MultiBinary(full_len)
        self.observation_space = obs_space                # Dict(…28…)

        # template of always-legal bits (28-3 = 25)
        self._mask_template = np.ones(full_len - 3, dtype=np.int8)
...
agent_name = "SB_MaskPPO_SF_AM_RM"
map_name = "V2_Base"
...
model = MaskablePPO(
    ...
    device=device,
    verbose=1,
    tensorboard_log=f"./tb_logs/MaskPPO/{map_name}/{agent_name}/"
)
