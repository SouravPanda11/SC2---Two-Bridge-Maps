...
MODEL_PATH = os.path.join(
    project_root, "Agents", "A2C", "saved_models", AGENT_NAME, f"{AGENT_NAME}_final.zip"
)
if not os.path.isfile(MODEL_PATH):
    sys.exit(f"[ERROR] Model file not found at: {MODEL_PATH}")

# ---------- directories ---------------------------
performance_root = os.path.join(project_root, "Agent Performance Charts", "A2C", AGENT_NAME)
os.makedirs(performance_root, exist_ok=True)

replay_root = os.path.join(project_root, "Replays", "A2C", AGENT_NAME)
...
# device setup
device = "cpu"
model = sb3.A2C.load(MODEL_PATH, env=env, device=device)
