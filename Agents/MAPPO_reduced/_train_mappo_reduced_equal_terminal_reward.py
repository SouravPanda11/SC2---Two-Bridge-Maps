"""
Isolated MAPPO driver for the equal terminal-success-reward experiment.

The established MAPPO implementation is reused without modifying it. During
this call its environment lookup is scoped to the two equal-25 modules and
...
AGENT_NAME = "MAPPO_reduced_equal_terminal_reward_25"

MAP_ENV_MODULES = {
    "V1_Base": (
        "Environments.MAPPO_reduced."
        "TB_env_MAPPO_reduced_equal_terminal_reward_V1_Base"
    ),
    "V2_Navigate": (
        "Environments.MAPPO_reduced."
        "TB_env_MAPPO_reduced_equal_terminal_reward_V2_Navigate"
    ),
}
...
def train_with_settings(map_name: str, **overrides):
    if map_name not in MAP_ENV_MODULES:
        ...
        raise ValueError(
            "Equal-reward MAPPO supports only "
    ...
    if not requested_agent_name.startswith(AGENT_NAME):
        raise ValueError(
            "Equal-reward outputs must use the isolated agent-name prefix "
            f"{AGENT_NAME!r}."
        )
