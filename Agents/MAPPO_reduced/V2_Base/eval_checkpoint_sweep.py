...
def get_agent_save_root():
    return project_root / "Agents" / "MAPPO_reduced" / MAP_NAME / "saved_models" / AGENT_NAME
...
def config_from_checkpoint(checkpoint, device):
    ...
    map_name = checkpoint_config.get("map_name", MAP_NAME)
...
def evaluate_checkpoint(env, checkpoint_path, eval_episodes, device, seed, greedy):
...
def main():
    ...
    include_player_relative = bool(manifest.get("include_player_relative", True))
    ...
    plot_combined(results_df, AGENT_NAME, MAP_NAME, combined_plot_path)
