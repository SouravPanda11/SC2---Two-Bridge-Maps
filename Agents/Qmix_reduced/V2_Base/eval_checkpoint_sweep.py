...
from Agents.Qmix_reduced.V2_Base.train_qmix import (
    MinimapEncoder,
    ParallelQMixEnvBatch,
    SharedQAgent,
    TrainConfig,
    safe_torch_load,
)
...
def get_agent_save_root(agent_name):
    return project_root / "Agents" / "Qmix_reduced" / MAP_NAME / "saved_models" / agent_name


def get_output_root(agent_name):
    return (
        project_root
...
class QMixReducedEvalPolicy:
    def __init__(self, checkpoint_path, env_info, device):
        self.device = device
        checkpoint = safe_torch_load(Path(checkpoint_path), map_location=device)
        self.config = config_from_checkpoint(checkpoint, device)
...
def create_eval_env(num_envs, seed, include_player_relative):
    env_kwargs = {
        "map_name": MAP_NAME,
        "episode_limit": None,
        "visualize": False,
...
def main():
    ...
    save_root = get_agent_save_root(args.agent_name)
    manifest_path, manifest = load_latest_seed_manifest(save_root)
    seeds = manifest["seeds"]
    output_root = get_output_root(args.agent_name)
    ...
    if first_run_config.is_file():
        ...
        with first_run_config.open("r", encoding="utf-8") as handle:
            include_player_relative = bool(
                json.load(handle).get("include_player_relative", include_player_relative)
            )
