"""
Isolated QMIX driver for the equal terminal-success-reward experiment.

The established per-map QMIX trainer is reused without modifying it. A
spawn-safe parallel environment batch loads only the new equal-25
...
AGENT_NAME = "QMIX_reduced_equal_terminal_reward_25"
...
MAP_ENV_MODULES = {
    "V1_Base": (
        ...
        "TB_env_QMIX_reduced_equal_terminal_reward_V1_Base"
    ),
    "V2_Navigate": (
        "Environments.QMIX_reduced."
        "TB_env_QMIX_reduced_equal_terminal_reward_V2_Navigate"
    ),
}
...
def qmix_equal_terminal_reward_env_worker(
    remote,
    parent_remote,
    rank: int,
    env_module: str,
    ...
    worker_tmp_dir = tempfile.mkdtemp(
        prefix=f"tbm-qmix-equal-reward-worker-{rank}-"
    )
...
def _make_parallel_env_batch(base_module):
    class ParallelQMixEqualTerminalRewardEnvBatch(
        ...
        def __init__(
            ...
            for rank in range(self.num_envs):
                ...
                process = self.ctx.Process(
                    target=qmix_equal_terminal_reward_env_worker,
                    args=(
                        worker_remote,
                        parent_remote,
                        rank,
    ...
    return ParallelQMixEqualTerminalRewardEnvBatch
...
def train_with_settings(
    *,
    ...
    if map_name not in TRAIN_MODULES:
        ...
        raise ValueError(
            "Equal-reward QMIX supports only "
