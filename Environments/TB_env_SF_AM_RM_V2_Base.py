...
class TwoBridgeEnv(gym.Env):
    ...
    def _shape_reward(self, vec, done, res):
        ...
        if (bx >= 0) and (by >= 0):
            ...
            nav_r = (self._prev_beacon_dists - beacon_dists)[f_alive].sum()
            self._prev_beacon_dists = beacon_dists

        # ─────────────────── COMBAT TERMS ───────────────────────────
        combat_r = 0.0

        # distance to enemy centroid
        if e_alive.any():
            cx, cy = ex[e_alive].mean(), ey[e_alive].mean()
            centroid_dists = np.hypot(fx - cx, fy - cy)
            if self._prev_centroid_dists is not None:
                combat_r += (self._prev_centroid_dists - centroid_dists)[f_alive].sum()
            self._prev_centroid_dists = centroid_dists
        else:
            # no enemies → clear the cache so next episode starts clean
            self._prev_centroid_dists = None
        ...
        # ───────────────────── TERMINAL BONUS ───────────────────────
        term_r = 10.0 if done and res in {"nav_win", "combat_win", "victory"} \
                 else -10.0 if done and "loss" in res else 0.0
